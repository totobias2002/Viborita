...
export const getCancellationPolicyLabel = (
  ...
  if (permiteCancelacionTardia) {
    return "Cancelacion online incluso fuera del limite";
  }

  if (cancelacionLimiteHoras === 0) {
    return "Cancelacion online hasta el horario de inicio";
  }
...
export class ViboritaApiClient {
  ...
  async listComplejos() {
    return this.request<ComplejoListItem[]>("/complejos", { method: "GET" });
  }
  ...
  async createReserva(payload: CreateReservaPayload, auth = false) {
